...
class CorreoResguardoExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithTitle, WithStyles
{
    ...
    {
        $query = DB::table('solicitud_correo as sc')
            ...
            ->select(
                ...
                'sc.puesto',
                'a.area',
                'sc.correo_institucional',
                'sc.usuario_generado',
                'sc.extension',
                'sc.fecha_activo'
            )
            ->where('sc.estatus', 'activo')
            ->whereNotNull('sc.fecha_activo');

        if ($this->del) {
            $query->where('sc.fecha_activo', '>=', $this->del . ' 00:00:00');
        }
        if ($this->al) {
            $query->where('sc.fecha_activo', '<=', $this->al . ' 23:59:59');
        }

        return $query->orderBy('sc.fecha_activo', 'desc')->get();
    }

    public function headings(): array
    {
        return ['ID', 'Nombre', 'Puesto', 'Área', 'Correo institucional', 'Usuario generado', 'Extensión', 'Fecha de activación'];
    }
